...
- **إصدار Xcode**: الـ workflow-ان **لا يُثبِّتان رقم إصدار Xcode**؛ يعتمدان على الإصدار الافتراضي المُختار مسبقًا على صورة الـ runner (`macos-latest`) — أول تجربة فعلية أظهرت أن تثبيت رقم محدَّد (كنا نستخدم `16.2` عبر `maxim-lobanov/setup-xcode`) يفشل بسرعة لأن أرقام Xcode المتوفرة تتغيّر باستمرار مع تحديث صور الـ runners (بحلول هذا التاريخ أصبحت أرقام Xcode بتنسيق سنوي مثل 26.x، واختفت إصدارات 16.x القديمة من بعض الصور). إن احتجت لاحقًا تثبيت إصدار محدَّد لسبب توافق معيّن، تحقّق أولًا من [قائمة البرامج المثبَّتة على صورة الـ runner الحالية](https://github.com/actions/runner-images) (ملف `macos-*-Readme.md`) قبل إضافة القيد.
